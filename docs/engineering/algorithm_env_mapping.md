...
| DQN | highway-v0 (discrete) | DiscreteMetaAction | Neural net replaces Q-table, experience replay, target network |
| Policy Gradient | highway-v0 (discrete) | DiscreteMetaAction | Policy gradient, weight log-prob by returns |
| PPO | racetrack-v0 | Continuous lateral steering | Clip mechanism, Gaussian policy for continuous control |
| SAC | racetrack-v0 | Continuous lateral steering | Maximum entropy framework, off-policy continuous control |
...
- **DQN / PG** use highway-v0 with discrete actions (lane change, accelerate, decelerate) to demonstrate how neural networks handle continuous state spaces.
- **PPO / SAC** use racetrack-v0 with continuous lateral steering to demonstrate real vehicle control — curves require precise continuous steering that discrete actions cannot handle.

## Highway-env Configuration

- **Discrete version** (DQN / PG): `highway-v0` with `DiscreteMetaAction` — 5 meta-actions
- **Continuous version** (PPO / SAC): `racetrack-v0` with `ContinuousAction` — lateral steering only
  - Observation: `[lat_off, ang_off, vx, vy, cos_h, sin_h]` (6-dim)
  - Action: `[steering]` (1-dim, range [-1, 1])
  - Reward: lane centering with strong off-road penalty
